    ...
    try {
      const payload = {
        planName: formData.planName,
          ...
          maxSecretary: formData.maxSecretary,
          features: formData.features.filter(f => f.trim() !== "")
      };

      const response = await fetch(
        process.env.NEXT_PUBLIC_API_URL + `/api/subscriptionPlan/add`,
        {
          method: "POST",
          headers: {
      ...
      >
        ...
        <div className="flex flex-col items-center gap-6">
          <div className="flex flex-row justify-start items-center w-full gap-4">
            <div>
              <h2 className="text-xl font-bold text-white">Add New Subscription Plan</h2>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="w-full space-y-4 text-sm">
            <div className="flex flex-col gap-2">
              <label htmlFor="planName" className="text-gray-200">
